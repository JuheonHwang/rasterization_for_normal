function [vertex, faces, texture] = read_obj(filename)
    ...
    if nargout > 2
        uv_lines = regexp(str,'vt [^\n]*\n', 'match');
        texture = zeros(length(uv_lines), 2);
        for i = 1:length(uv_lines)
            uv = sscanf(uv_lines{i}, 'vt %f %f');
            texture(i, :) = uv';
        end
    end
